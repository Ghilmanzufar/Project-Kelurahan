    ...
    <div x-data="{ 
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <div class="p-6 border-b border-gray-200">
                ...
                {{-- =============================================== --}}
                {{-- <<< TAMBAHKAN FORM SEARCH BAR INI >>> --}}
                {{-- =============================================== --}}
                <form action="{{ route('admin.petugas.index') }}" method="GET" class="mt-4">
                    <div class="flex space-x-2">
                        ...
                        @if(request('search'))
                            <a href="{{ route('admin.petugas.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-400">
                                Reset
                            </a>
                        @endif
                    </div>
                            ...
                            @forelse ($allPetugas as $petugas)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        ...
                                        {{-- =============================================== --}}
                                        <form x-ref="formHapusPetugas{{ $petugas->id }}" 
                                              action="{{ route('admin.petugas.destroy', $petugas->id) }}" 
                                              method="POST" class="inline"
                                              @submit.prevent="
                                                confirmMessage = 'Apakah Anda yakin ingin menghapus akun: \'{{ $petugas->nama_lengkap }}\'? Aksi ini tidak dapat dibatalkan.'; 
                                                confirmAction = () => { $refs.formHapusPetugas{{ $petugas->id }}.submit(); showConfirmModal = false; }; 
                                            ...
                                            <button type="submit" 
                                                    ...
                                                    {{-- Nonaktifkan tombol hapus jika itu akun Anda sendiri --}}
                                                    @if(Auth::id() == $petugas->id) disabled @endif>
                                                Hapus
                                            </button>
                                        </form>
                                        {{-- =============================================== --}}
